...
def lista_deseos():
    ...
    usuario = obtener_usuario_actual()
    return render_template('lista_deseos.html', deseos_items=productos, usuario=usuario)
...
@app.route('/deseos/eliminar/<deseo_id>', methods=['POST'])
def eliminar_deseo(deseo_id):
    if not usuario_logueado():
        flash("Debe iniciar sesión para modificar la lista de deseos.", "warning")
        return redirect(url_for('login'))

    mongo.db.deseos.delete_one({'usuario_id': session['usuario_id'], 'producto_id': deseo_id})
...
def agregar_carrito(producto_id):
    ...
    if producto_id in carrito:
        ...
        carrito[producto_id] = {
            'productocarrito_id': str(producto['_id']),
    ...
    flash(f"Producto '{producto['titulo']}' agregado al carrito.", "success")

    return redirect(request.referrer or url_for('index'))


@app.route('/carrito/eliminar/<carrito_id>', methods=['POST','GET'])
def eliminar_carrito(carrito_id):
    ...
    key = f"carrito:{user_id}"
    redis_conn.hdel(key, carrito_id)
...
def actualizar_cantidad_carrito(item_id):
    try:
        ...
        key = f"carrito:{user_id}"
        item_json = redis_conn.hget(key,item_id)
        if not item_json:
        ...
        if accion == 'incrementar':
            item['cantidad'] +=1
            redis_conn.hset(key, item_id, json.dumps(item))
...
def vaciar_deseos():
    try:
        ...
        resultado = mongo.db.deseos.delete_many({'usuario_id': user_id})
    ...
    try:
        ...
        if deseos_items:
            ...
            for producto in productos:
                carrito_item = mongo.db.Carrito.query.filter_by(user_id=user_id, producto_id=producto.id).first()
                if carrito_item:
                    carrito_item.cantidad += 1
                else:
                    nuevo_item = mongo.db.Carrito(user_id=user_id, producto_id=producto.id, cantidad=1)
                    mongo.db.session.add(nuevo_item)
